...
{
 private readonly IFocusTimerService stopwatch; private readonly PomodoroTimerService pomodoro; private readonly IFocusSessionService sessions;
 public FocusPageViewModel(IReadOnlyList<TaskItem> tasks,IFocusTimerService stopwatch,PomodoroTimerService pomodoro,IFocusSessionService sessions){Tasks=tasks.Where(t=>!t.IsCompleted&&t.QuadrantId is not null).ToArray();this.stopwatch=stopwatch;this.pomodoro=pomodoro;this.sessions=sessions;}
 ...
 public async Task ActivateAsync(){var s=await sessions.GetCurrentAsync();Status=s?.Status;Refresh();}
